...
class CallGraphProcessor(ProcessingBase):
    def __init__(
        ...
        logger.debug(
            f"In CallGraphProcessor.__init__: filename: {filename}; mod_name: {modname}; "
            f" call_graph: {str(call_graph)}; analyzed modules: {str(modules_analyzed)}"
        )
    ...
    def add_to_current_func(self, line_number: int):
            ...
            for name in names:
                ...
                if self.def_manager.get(iter_ns):
                    self.call_graph.add_edge(
                        self.current_method, iter_ns, mod=self.modname
                    )
                if self.def_manager.get(next_ns):
                    self.call_graph.add_edge(
                        self.current_method, next_ns, mod=self.modname
                    )
    ...
    def visit_Raise(self, node: ast.Raise):
        logger.debug(
            "In CallGraphProcessor.visit_Raise line number: %d-- %s"
            % (node.lineno, self.current_method)
        )
        self.add_to_current_func(node.lineno)
        if not node.exc:
        ...
        for d in decoded:
            ...
            for name in names:
                ...
                if pointer_def.is_class_def():
                    ...
                    for ns in init_ns:
                        self.call_graph.add_edge(
                            self.current_method, ns, mod=self.modname
                        )
                if pointer_def.is_ext_def():
                    self.call_graph.add_edge(
                        self.current_method, name, mod=self.modname
                    )
        # logger.debug("Exit CallGraphProcessor.visit_Raise")

    def visit_AsyncFunctionDef(self, node: ast.AsyncFunctionDef):
        logger.debug(
            "In CallGraphProcessor.visit_AsyncFunctionDef: line number: %d -- %s"
            % (node.lineno, self.current_method)
        )
    ...
    def visit_FunctionDef(self, node: Union[ast.FunctionDef, ast.AsyncFunctionDef]):
        logger.debug(
            "In CallGraphProcessor.visit_FunctionDef: line number: %d -- %s"
            % (node.lineno, self.current_method)
        )
        ...
        self.call_graph.cg_extended[utils.join_ns(self.current_ns, node.name)]["meta"][
            "lineno"
        ] = node.lineno
        arg_names = []
        default_vals = []
    ...
    def visit_Raise(self, node: ast.Raise):
        ...
        if isinstance(node.exc, ast.Call):
            ...
            if isinstance(node.exc.func, ast.Name):
                ...
                if (
                    self.current_node_name is not None
                    and FTS in self.call_graph.cg_extended
                ):
                    try:
                        # logger.info("Adding raise 1")
                        self.call_graph.cg_extended[FTS]["meta"]["raises"].add(
                            node.exc.func.id
                        )
    ...
    def visit_If(self, node: ast.If):
        ...
        if self.current_node_name is not None and FTS in self.call_graph.cg_extended:
            try:
    ...
    def visit_Call(self, node: ast.Call):
        ...
        def create_ext_edge(name, ext_modname, e_lineno=-1, mod=""):
            ...
            self.call_graph.add_edge(
                self.current_method, name, lineno=e_lineno, mod=mod, ext_mod=ext_modname
            )
        ...
        # logger.debug(
        #     "In CallGraphProcessor.visit_Call: Iterating node with line number: %d"
        #     % node.lineno
        # )
        ...
        try:
            if (
                ...
                and node.func.value.id == "atheris"
        ...
        if not names:
            ...
            if isinstance(node.func, ast.Attribute) and self.has_ext_parent(node.func):
                ...
                create_ext_edge(
                    name, utils.constants.BUILTIN_NAME, node.lineno, self.modname
                )
                ...
                try:
                    ...
                    # a3 = node.func.attr
                    # logger.debug(
                    #     "In CallGraphProcessor.visit_Call: Retrieved function call name: %s"
                    #     % (lhs)
                    # )
                    ...
                    if "self." not in lhs:
                        ...
                        create_ext_edge(
                            lhs, utils.constants.BUILTIN_NAME, node.lineno, self.modname
                        )
                except Exception as e:
                    # logger.error(
                    #     "In CallGraphProcessor.visit_Call: Exception: %s" % str(e)
                    # )
                    pass
            # logger.debug("I-4")
            # logger.debug(
            #     "Exit CallGraphProcessor.visit_Call: No name definition found: Fail safe logic"
            # )
            return
        ...
        for pointer in names:
            ...
            if pointer_def.is_callable():
                if pointer_def.is_ext_def():
                    ...
                    continue
                self.call_graph.add_edge(
                    self.current_method, pointer, lineno=node.lineno, mod=self.modname
                )
            ...
            if pointer_def.is_class_def():
                ...
                for ns in init_ns:
                    self.call_graph.add_edge(
                        self.current_method, ns, lineno=node.lineno, mod=self.modname
                    )
    ...
    def get_all_reachable_functions(self) -> set:
    ...
    def has_ext_parent(self, node: ast.AST) -> bool:
            ...
            for parent in parents:
                for name in self.closured.get(parent, []):
                    ...
                    if defi and defi.is_ext_def():
                        # logger.debug(
                        #     "Exit CallGraphProcessor.has_ext_parent: External parent found"
                        # )
                        return True
            node = node.value
    ...
    def get_full_attr_names(self, node: ast.Attribute):
