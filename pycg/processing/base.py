...
class ProcessingBase(ast.NodeVisitor):
    ...
    def __init__(self, filename: str, modname: str, modules_analyzed: Set[str]) -> None:
        logger.debug(
            f"In ProcessingBase.__init__: filename: {filename}; mod_name: {modname}; "
            f" analyzed modules: {str(modules_analyzed)}"
        )
    ...
    def visit_ClassDef(self, node: ast.ClassDef):
        ...
        self.name_stack.append(node.name)
        self.method_stack.append(node.name)
        if self.scope_manager.get_scope(self.current_ns) is not None:
            self.scope_manager.get_scope(self.current_ns).reset_counters()
            for stmt in node.body:
                self.visit(stmt)
        self.method_stack.pop()
    ...
    def _get_target_ns(self, target):
        ...
        if isinstance(target, ast.Attribute):
            ...
            res = [utils.join_ns(base, target.attr) for base in bases]
    ...
    def _visit_assign(self, value, targets):
        ...
        def do_assign(decoded, target):
            # logger.debug(
            #     "In ProcessingBase._visit_assign.do_assign: Target: %s" % target
            # )
            self.visit(target)
    ...
    def decode_node(self, node) -> List[Definition]:
        ...
        if isinstance(node, ast.Name):
            ...
            for called_def in decoded:
                ...
                if called_def.is_function_def():
                    return_ns = utils.join_ns(
                        called_def.fullns, utils.constants.RETURN_NAME
                    )
            ...
            decoded = [self.decode_node(elt) for elt in node.elts]
            ...
            self.scope_manager.get_def(self.current_ns, dict_name)
            ...
            self.scope_manager.get_def(self.current_ns, list_name)
    ...
    def analyze_submodule(self, cls, imp, *args, **kwargs) -> None:
        ...
        if not fname or self.import_manager.get_mod_dir() not in fname:
            return
